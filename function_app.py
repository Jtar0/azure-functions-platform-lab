...
def howdy(req: func.HttpRequest) -> func.HttpResponse:
    ...
    if name:
        return func.HttpResponse(f"Howdy, {name}! Deployed automatically through GitHub Actions.")
    else:
...
# @app.blob_trigger(arg_name="client", path="uploads/{name}",
#                   connection="AzureWebJobsStorage")
# def process_upload(client: blob.BlobClient):
#     logging.info(
